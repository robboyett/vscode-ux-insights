...
// src/extension.ts
async function getMarkdownFilesFromPath(rootPath, folder) {
  const files = [];
...
function autoDetectResearchFolders(rootPath) {
  ...
  const detected = [];
  try {
    const entries = fs2.readdirSync(rootPath, { withFileTypes: true });
    for (const entry of entries) {
      if (entry.isDirectory()) {
        const name = entry.name.toLowerCase();
        if (keywords.some((k2) => name.includes(k2))) {
          detected.push("./" + entry.name);
        }
      }
    }
    if (entries.some((e) => e.isDirectory() && e.name === "docs")) {
      const docsEntries = fs2.readdirSync(path.join(rootPath, "docs"), { withFileTypes: true });
...
async function discoverResearchFiles() {
  ...
  if (autoDetect) {
    ...
    searchPaths = Array.from(/* @__PURE__ */ new Set([...searchPaths, ...detectedPaths]));
  }
  let allFiles = [];
  ...
  return allFiles.sort((a3, b2) => b2.lastModified.getTime() - a3.lastModified.getTime());
}
...
function fuzzyMatchScore(openFile, researchFile) {
  const openBase = openFile.split(/[\\/]/).pop()?.split(".")[0] || "";
  const openWords = openBase.toLowerCase().split(/[^a-z0-9]+/).filter((w2) => w2.length >= 3 && !STOP_WORDS.has(w2));
  const researchWords = (researchFile.title + " " + researchFile.path).toLowerCase().split(/[^a-z0-9]+/).filter((w2) => w2.length >= 3 && !STOP_WORDS.has(w2));
...
var SectionHeaderTreeItem = class extends vscode.TreeItem {
  constructor(label) {
...
var ResearchFileTreeItem = class extends vscode.TreeItem {
  constructor(file) {
...
var RefreshTreeItem = class extends vscode.TreeItem {
  constructor() {
...
var ResearchInsightsProvider = class {
  ...
  async refresh() {
    ...
    this.researchFiles.sort((a3, b2) => b2.lastModified.getTime() - a3.lastModified.getTime());
  ...
  async updateRelevance() {
    ...
    if (!openFile) {
      ...
      this.relevantFiles = scored.filter((s) => s.score > 0).map((s) => s.f);
    }
    this._onDidChangeTreeData.fire();
  ...
  async getChildren(element) {
    if (!element) {
      const items = [new RefreshTreeItem()];
      if (this.relevantFiles.length > 0) {
...
var ResearchPanel = class _ResearchPanel {
  static currentPanel;
  _panel;
  _extensionUri;
  _disposables = [];
  constructor(panel, extensionUri, filePath) {
    this._panel = panel;
    this._extensionUri = extensionUri;
    this._update(filePath);
    this._panel.onDidDispose(() => this.dispose(), null, this._disposables);
  }
  static createOrShow(extensionUri, filePath) {
    const column = vscode.window.activeTextEditor ? vscode.window.activeTextEditor.viewColumn : void 0;
    if (_ResearchPanel.currentPanel) {
      _ResearchPanel.currentPanel._panel.reveal(column);
      _ResearchPanel.currentPanel._update(filePath);
    } else {
      ...
      _ResearchPanel.currentPanel = new _ResearchPanel(panel, extensionUri, filePath);
    }
  }
  dispose() {
    _ResearchPanel.currentPanel = void 0;
    this._panel.dispose();
...
async function showResearchFileSelector(context) {
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("ux-insights.openResearchFile", (filePath) => {
